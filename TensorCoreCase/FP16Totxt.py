...
def MatABCD2Register(a, b, c, d, thread: int = 32, xLen=64):
...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(result_path, 'a.txt')):
        ...
        a, b, c, d = getMatABCD(8, 8, 8,result_path)
        print(getFP16Str(0))
        print("Get new data.")
    ...
    # print(a_str)
    print(len(a_str))
    RA, RB, RC, RD = MatABCD2Register(a_str, b_str, c_str, d_str,thread=32,xLen=32)
    strArr_Save(RA, os.path.join(result_path, 'RA.txt'))
    strArr_Save(RB, os.path.join(result_path, 'RB.txt'))
    strArr_Save(RC, os.path.join(result_path, 'RC.txt'))
    strArr_Save(RD, os.path.join(result_path, 'RD.txt'))
